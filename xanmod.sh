...
if grep -q "CPU supports x86-64-v4" <<< "$(/bin/bash check_x86-64_psabi.sh)"; then
  echo "CPU 支持 x86-64-v4，安装 linux-xanmod-x64v4"
  sudo apt install -y linux-xanmod-x64v4
elif grep -q "CPU supports x86-64-v3" <<< "$(/bin/bash check_x86-64_psabi.sh)"; then
  echo "CPU 支持 x86-64-v3，安装 linux-xanmod-x64v3"
  sudo apt install -y linux-xanmod-x64v3
elif grep -q "CPU supports x86-64-v2" <<< "$(/bin/bash check_x86-64_psabi.sh)"; then
